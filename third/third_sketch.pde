...
 if(xBall + 25 > width || xBall - 25 < 0) //if ball hits left or right side of the screen, ball moves in opposite direction
 {
   xSpeed = xSpeed * -1; //invert sign of x-speed, ball moves in oppostite direction
   score = score - 1; //decrease score by 1
 }

 if(yBall + 25 > height || yBall - 25 < 0) //if ball hits top or bottom of the screen, ball moves in opposite direction
 {
   ySpeed = ySpeed * -1;  //invert sign of y-speed, ball moves in oppostite direction
   score = score - 1; //decrease score by 1
 }

 if(score < 0) //if score is less than zero
 {
   score = 10; //set score back to ten
 }
}
